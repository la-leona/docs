# 개념 정리 — Redis / Kafka / OAuth

- 작성일: 2026-07-31
- 목적: 세 기술의 **개념**을 잡고, **lgoneid 프로젝트에서 실제로 어디에 쓰이는지** 연결해서 이해하기.
...
## ⚠️ 우리가 실제로 당한 함정
**DB 공통코드를 UPDATE 했는데 앱 동작이 안 바뀜** — 앱은 Redis 캐시를 읽기 때문. 갱신하려면 배치 잡 호출:
```
/adminbatch/job/refreshCommoncodeRedisCacheJob
```
→ 캐시의 본질적 트레이드오프다. **빠른 대신 "원본과 잠깐 다를 수 있다"(stale)**. 그래서 "DB는 바꿨는데 앱은 옛 값" 증상이면 **캐시를 1순위로 의심**해야 한다.
...
| 갱신 주체 | `admin-service/.../commoncode/controller/CommonCodeRedisCacheController` 의 엔드포인트 3개 — `refreshCommonCodeRedisCache`(=삭제 후 재적재) / `createRedisCacheCommonCode` / `deleteCommonCodeCache` |
...
→ 즉 **DB를 직접 바꿨든 화면에서 바꿨든, refresh 를 따로 호출해야 한다.** (설계 의도가 그런지, 누락인지는 미확인 — 원 담당 확인 필요)
...
[사용자] ──OAuth──> [네이버/카카오] ──code/token──> [user-web]
                                    ...
                                    (1) 공통코드 스위치 읽기 ─────> [Redis 캐시] (admin)
                                                      |
                                    (2) 회원 조회/연동 ──Feign──> [member-service] ──> [DB]
                                                      |
