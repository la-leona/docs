...
우리 프로젝트 실제 흐름(2026-08-04 기준). 필요하면 그대로 복사해 쓰면 된다.
...
sequenceDiagram
    autonumber
    actor U as 사용자
    participant P as 로그인화면(부모창)
    participant SN as SnsNaverController
    participant NV as 네이버 OAuth
    participant LJ as LoginJspController
    participant MS as member-service
    participant AF as affiliates-service
    participant AT as apptest(모의 계열사)

    U->>P: 네이버 버튼 클릭
    P->>SN: GET /sns/naverLogin (팝업)
    SN->>NV: authorize 리다이렉트
    NV-->>SN: 콜백 (code)
    SN->>NV: 토큰 발급 + 프로필 조회
    NV-->>SN: access_token, 프로필(CI 포함)
    Note over SN: SnsLoginDto 세션 저장<br/>팝업 close → 부모창 호출
    P->>LJ: POST /login/snsChk
    LJ->>MS: 기존회원 후보 조회<br/>(CI 또는 (전화/이메일)+이름+성별+생년)
    MS-->>LJ: 후보 목록
    ...
    alt 이미 이 소셜과 연계됨
        ...
        LJ-->>P: 후보 목록 화면
        U->>P: 계정 선택 + 비밀번호 확인
        P->>MS: TB_MEM_SNS 연계 생성
    else 후보 없음 (신규)
        LJ-->>P: 신규가입 (jnKndCd=SIMUL_JN)
        U->>P: 약관 동의(MLI+계열사) → 정보 입력
        P->>MS: 회원 생성 (TB_MEM 계열)
        P->>AF: S13 계열사 전달
        AF->>AT: 회원등록+매핑+약관
        AT-->>AF: affMemCd
        AF-->>P: 결과
        P->>AF: 매핑 저장 (TB_AFF_SVC_MEM)
    end
```
````
